...
void first_come_first_deal(FILE * fp)
{
    ...
    int time = 0;
    while(jobqueue!=NULL||readyqueue!=NULL||CPU!=NULL)
    {
        printf("now_time:%d\n",time);

        while((jobqueue!=NULL) && (jobqueue->arrival_time==time))
        {
        ...
        if(CPU!=NULL)
        {
            if((CPU->cpu_time + CPU->burst_time) ==time)
            {
